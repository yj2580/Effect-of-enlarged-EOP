...
There are some missing values in eop_size_mm and fhp_size_mm, which truly equal 0, so we should replace them with 0. Then I recode sex and age_group with illustrative characters. Finally, create some ordered factor for categorical variables. In case of missing 14.6 in eop_size and 30.8 in fhp_category in subsequent analysis, I add them into levels.
...
eop_size = group eop_size_mm for every 5 mm, useful to identify EEOP
eop_visibility_classification = visible classification, the threshold for recording the size of an EOP was set at 5mm
...
From the table and graph aboved, we can see that sex distribution is almost even amongst age. Age distribution is nearly even from 18 to 72, although the amount is extremely high for group 61-year-old male and 51-year-old female. We can also see a slight right skew in age distribution. (it will be even if grouped by age group, because ">60" group has a wider range.)
