...
There are some missing values in eop\_size\_mm and fhp\_size\_mm, which
truly equal 0, so we should replace them with 0. Then I recode sex and
age\_group with illustrative characters. Finally, create some ordered
factor for categorical variables. In case of missing 14.6 in eop\_size
and 30.8 in fhp\_category in subsequent analysis, I add them into
levels. There are 1221 participants and 9 variables. age\_group = age
groups according to decades eop\_size\_mm = the size of the external
occipital protuberance eop\_size = group eop\_size\_mm for every 5 mm,
useful to identify EEOP eop\_visibility\_classification = visible
classification, the threshold for recording the size of an EOP was set
at 5mm fhp\_size\_mm = the size of forward head protraction
fhp\_category = group fhp\_size\_mm for every 10 mm
...
From the table and graph aboved, we can see that sex distribution is
almost even amongst age. Age distribution is nearly even from 18 to 72,
although the amount is extremely high for group 61-year-old male and
51-year-old female. We can also see a slight right skew in age
distribution. (it will be even if grouped by age group, because “\>60”
group has a wider range.)
